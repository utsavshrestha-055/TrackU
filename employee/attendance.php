...
<html lang="en">
  ...
  <body>
    ...
    <form method="POST" action="../database/attendance.php" class="form-box">
        <div class="allign">
              <h2><b>Bus Number: <?php  echo $rows['sbus'];?></b></h2>
             
          <?php
              $bus=$rows['sbus'];
              $sql = "SELECT * FROM details WHERE sbus='$bus'";
              ...
              if (mysqli_num_rows($result)>0){
                  ...
                  {
                    ?>
                <input type="checkbox" name ="list[]" value="<?= $rows['cname'] ;?>" ></input><?= $rows['cname'] ;?><br>
<?php
                  }}
?>
             
       <button type="submit" name="attendance_button" class="btn">Submit</button>
        </div>
                </form>
